(function() {
  var DIRECTIVE, EXPLICIT_PATH, HEADER, REMOTE_PATH, assetsMiddleware, cache, collectDependencies, compilers, concatenate, createHelpers, crypto, dependencies, directivesInCode, fs, generateTags, jsProductionUrl, libs, md5Suffix, mime, minify, parse, path, prepareJs, productionJsPath, readFileOrCompile, relPath, sendCallback, sendFile, serveCompiled, serveRaw, stripSuffix, uglify, updateDependenciesSync;
  ...
  productionJsPath = function(filePath, suffix) {
    if (suffix == null) {
      suffix = '';
    }
    return filePath.replace(/\.js$/, ".complete" + suffix + ".js");
  };
  stripSuffix = function(filePath) {
    return filePath.replace(/-[a-f0-9]*\.js$/, ".js");
  };
  jsProductionUrl = function(jsUrl, options) {
    var filePath, jsPath, _ref;
    filePath = path.join(options.src, jsUrl);
    jsPath = productionJsPath(filePath);
        if ((_ref = cache[jsPath]) != null) {
      _ref;
    } else {
      cache[jsPath] = prepareJs(filePath, options);
    };
    return productionJsPath(filePath, cache[jsPath].suffix);
  };
  prepareJs = function(filePath, options) {
    var str, suffix;
  ...
  createHelpers = function(options) {
    ...
    context.js = function(jsPath) {
      var dependencyTags, filePath, jsUrl, packagePath, tag;
