(function() {
  ...
  updateDependenciesSync = function(filePath, options) {
    var c, depPath, directive, directivePath, ext, jsCompilerExts, jsExtList, localDependencies, oldTime, requireTree, words, _i, _j, _len, _len2, _ref, _ref2, _ref3;
    if (filePath.match(REMOTE_PATH)) return dependencies[filePath] = [];
    oldTime = (_ref = cache[filePath]) != null ? _ref.mtime : void 0;
    ...
    if (cache[filePath].mtime.getTime() === (oldTime != null ? oldTime.getTime() : void 0)) {
      if (dependencies[filePath]) return;
    }
    for (ext in compilers) {
      c = compilers[ext];
    ...
    localDependencies = [];
    _ref2 = directivesInCode(cache[directivePath].str);
      ...
      switch (words[0]) {
        ...
        case 'require_tree':
          requireTree = function(parentDir, paths) {
            var p, stats, _k, _len3, _ref4, _results;
            _results = [];
            for (_k = 0, _len3 = paths.length; _k < _len3; _k++) {
              ...
              stats = fs.statSync(p);
              if (stats.isFile()) {
                if (_ref4 = path.extname(p), __indexOf.call(jsExtList, _ref4) < 0) {
                  continue;
                }
                if (path.extname(p) !== '.js') p = p.replace(/[^.]+$/, 'js');
                updateDependenciesSync(p);
                _results.push(localDependencies.push(p));
    ...
    return dependencies[filePath] = localDependencies;
  };
