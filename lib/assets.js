...
(function() {
  ...
  ConnectAssets = (function() {
    ...
    ConnectAssets.prototype.createHelpers = function() {
      ...
      context.css = function(route, pathsOnly) {
        ...
        if (_this.options.pathsOnly || pathsOnly) {
          return route;
        }
        return "<link rel='stylesheet' href='" + route + "'>";
      };
      context.css.root = 'css';
      context.js = function(route, pathsOnly) {
        var p, r, routes;
    ...
    ConnectAssets.prototype.cacheImg = function(route) {
      ...
      try {
        ...
        if (timeEq(stats.mtime, (_ref = this.cache.map[route]) != null ? _ref.mtime : void 0)) {
          alreadyCached = true;
        } else {
          mtime = stats.mtime;
    ...
    ConnectAssets.prototype.fixCSSImagePaths = function(css) {
      var regex, resolveImgPath,
        _this = this;
      resolveImgPath = function(path) {
        var absPrefix, relativePrefix, relativePrefixExp, resolvedPath;
        resolvedPath = path + "";
        resolvedPath = resolvedPath.replace(/url\(|'|"|\?.*['"\)]|\)/g, '');
        if (_this.options.servePath) {
          absPrefix = "" + _this.options.servePath + "/" + _this.options.helperContext.img.root + "/";
          if (resolvedPath.indexOf(absPrefix) === 0) {
            resolvedPath = resolvedPath.substring(absPrefix.length);
          } else {
            relativePrefix = "" + (parse(_this.options.servePath).pathname.replace(/\/$/, '')) + "/" + _this.options.helperContext.img.root + "/";
            relativePrefixExp = new RegExp('^' + relativePrefix.replace(/\//g, '\\/'));
            resolvedPath = resolvedPath.replace(relativePrefixExp, '');
          }
        }
        resolvedPath = resolvedPath.replace(new RegExp("^\\.\\.\\/" + (_this.options.helperContext.img.root.replace(/\//g, '\\/')) + "\\/"), '');
        try {
          resolvedPath = _this.options.helperContext.img(resolvedPath);
        } catch (e) {
    ...
    ConnectAssets.prototype.compileCSS = function(route) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        try {
          stats = fs.statSync(this.absPath(sourcePath));
          if (ext === 'css') {
            if (timeEq(stats.mtime, (_ref1 = this.cache.map[route]) != null ? _ref1.mtime : void 0)) {
              alreadyCached = true;
            } else {
              mtime = stats.mtime;
