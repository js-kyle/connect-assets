(function() {
  var DIRECTIVE, EXPLICIT_PATH, HEADER, REMOTE_PATH, assetsMiddleware, cache, compilers, concatenate, createHelpers, dependencies, directivesInCode, fs, generateTags, libs, mime, minify, parse, path, productionPath, readFileOrCompile, relPath, sendCallback, serveCompiled, serveRaw, traverseDependencies, uglify, updateDependenciesSync;
  ...
  traverseDependencies = function(filePath, traversedPaths, callback) {
    var depPath, _i, _len, _ref, _results;
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      _results.push(callback(depPath));
    }
    return _results;
  };
  generateTags = function(filePath, options, loadedDeps) {
    var tags;
    if (loadedDeps == null) loadedDeps = [];
    if (filePath.match(REMOTE_PATH)) {
      return ["<script src='" + filePath + "'></script>"];
    }
    tags = [];
    traverseDependencies(filePath, loadedDeps, function(depPath) {
      return tags = tags.concat(generateTags(depPath, options, loadedDeps));
    });
    tags.push("<script src='" + (relPath(options.src, filePath)) + "'></script>");
    return tags;
  };
  concatenate = function(filePath, options, loadedDeps) {
    var script;
    if (loadedDeps == null) loadedDeps = [];
    script = '';
    traverseDependencies(filePath, loadedDeps, function(depPath) {
      return script += concatenate(depPath, options, loadedDeps);
    });
