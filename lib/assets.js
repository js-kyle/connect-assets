(function() {
  var DIRECTIVE, EXPLICIT_PATH, HEADER, REMOTE_PATH, assetsMiddleware, cache, compilers, createHelpers, dependencies, directivesInCode, fs, libs, mime, parse, path, sendCallback, serveCompiled, serveRaw, updateDependenciesSync;
  fs = require('fs');
  mime = require('mime');
  path = require('path');
  ...
  createHelpers = function(options) {
    ...
    context.js = function(jsPath) {
      var assetPath, depPath, dependencyTags;
      jsPath = expandPath(jsPath, jsExt, context.js.root);
      if (context.js.concatenate) {
        return "<script src='" + jsPath + "'></script>";
      } else {
        dependencyTags = '';
        if (options.src && !jsPath.match(REMOTE_PATH)) {
          assetPath = path.join(options.src, jsPath);
          if (updateDependenciesSync(assetPath, jsPath)) {
            dependencyTags = ((function() {
              var _i, _len, _ref, _results;
              _ref = dependencies[assetPath];
              _results = [];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                depPath = _ref[_i];
                _results.push("<script src='" + depPath + "'></script>");
              }
              return _results;
            })()).join('\n') + '\n';
          }
        }
        return "" + dependencyTags + "<script src='" + jsPath + "'></script>";
      }
    };
    context.js.root = '/js';
  ...
  HEADER = /(?:(\#\#\#.*\#\#\#\n?)|(\\\/\\\/.*\n?)+|(\#.*\n?)+)+/;
  DIRECTIVE = /^[\W]*=\s*(\w+.*?)(\*\\\/)?$/m;
  updateDependenciesSync = function(filePath, hrefPath) {
    var compiler, depPath, directive, directives, ext, fallbackPath, processFile, words, _i, _j, _len, _len2, _ref;
    processFile = function(filePath) {
      var stats, str, _ref;
      stats = fs.statSync(filePath);
      if (((_ref = cache[filePath]) != null ? _ref.mtime : void 0) === stats.mtime) {
        return null;
      }
  ...
  directivesInCode = function(code) {
    ...
    _ref = HEADER.exec(code);
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      header = _ref[_i];
      if (header != null) {
        _results.push(DIRECTIVE.exec(header)[1]);
      }
    }
    return _results;
  };
