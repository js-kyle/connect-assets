(function() {
  var cache, compilers, fs, libs, mime, parse, path, safe_require, sendStr, serveCompiled, serveRaw;
  fs = require('fs');
  mime = require('mime');
  path = require('path');
  ...
  compilers = {
    ...
    styl: {
      ...
      compile: function(filepath, callback) {
        ...
        if ((_ref = libs.nib) == null) {
          libs.nib = safe_require('nib', (function() {
            return function() {};
          }));
        }
        return fs.readFile(filepath, 'utf8', function(err, str) {
          return libs.stylus(str).use(libs.nib()).set('filename', filepath).render(callback);
        });
      }
    }
  };
  safe_require = function(m, alt) {
    try {
      return require(m);
    } catch (err) {
      return alt;
