...
Assets.prototype.serveAsset = function (req, res, next) {
  ...
  if (isPossibleSourceMapPath(path)) {
    // Check that this is actually a source map request and not an asset request that ends with
    // the .map extension.
    if (!this.isValidAssetPath(path)) {

      // The asset doesn't exist with the .map extension. Attempted to serve source map.
      try {
        var originalAssetPath = parts.path.replace(".map", "")
        if (!this.options.compile) {
          assetName = this.manifest.assets[originalAssetPath];
          asset = this.manifest.files[assetName];
        }
        else {
          asset = this.environment.findAsset(originalAssetPath, { bundle: this.options.build });
        }
      } catch (err) {
        return next(err);
      }
...
Assets.prototype.isValidAssetPath = function (pathname) {
  if (!this.options.compile) {
    assetName = this.manifest.assets[pathname];
    return !!this.manifest.files[assetName];
  }
  else {
    return !!this.environment.findAsset(pathname);
  }
}
